...
function hexOrBuffer(valueInput, name) {
    ...
    if (!Buffer.isBuffer(value)) {
        if (!isHexString(value)) {
            const error = new Error(name ? (`[nio-abi] invalid ${name}`) : '[nio-abi] invalid hex or buffer, must be a prefixed alphanumeric even length hex string');
            error.reason = '[nio-abi] invalid hex string, hex must be prefixed and alphanumeric (e.g. 0x023..)';
            error.value = value;
            throw error;
        }
...
function getKeys(params, key, allowEmpty) {
    ...
    if (!Array.isArray(params)) { throw new Error(`[nio-abi] while getting keys, invalid params value ${JSON.stringify(params)}`); }
    ...
    for (var i = 0; i < params.length; i++) { // eslint-disable-line
        ...
        if (allowEmpty && !value) {
            ...
            throw new Error('[nio-abi] while getKeys found invalid ABI data structure, type value not string');
        }
        result.push(value);
    }
...
function coderFixedBytes(length) {
    return {
        ...
        decode: function decodeFixedBytes(data, offset) {
            if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[nio-abi] while decoding fixed bytes, invalid bytes data length: ${length}`); }
...
const coderAddress = {
    encode: function encodeAddress(valueInput) {
        ...
        var result = Buffer.alloc(32); // eslint-disable-line
        if (!isHexString(value, 32)) { throw new Error('[nio-abi] while encoding address, invalid address value, not alphanumeric 32 byte hex string'); }
        value = hexOrBuffer(value);
        result.fill(0);
        value.copy(result, 0);
        return result;
    ...
    decode: function decodeAddress(data, offset) {
        ...
        if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[nio-abi] while decoding address data, invalid address data, invalid byte length ${data.length}`); }
        return {
            consumed: 32,
...
function decodeDynamicBytesHelper(data, offset) {
    if (data.length !== 0 && data.length < offset + 32) { throw new Error(`[nio-abi] while decoding dynamic bytes data, invalid bytes length: ${data.length} should be less than ${offset + 32}`); }

    var length = uint256Coder.decode(data, offset).value; // eslint-disable-line
    length = length.toNumber();
    if (data.length !== 0 && data.length < offset + 32 + length) { throw new Error(`[nio-abi] while decoding dynamic bytes data, invalid bytes length: ${data.length} should be less than ${offset + 32 + length}`); }
...
function coderArray(coder, lengthInput) {
    return {
        encode: function encodeArray(value) {
            ...
            if (!Array.isArray(value)) { throw new Error('[nio-abi] while encoding array, invalid array data, not type Object (Array)'); }
            ...
            if (length !== value.length) { throw new Error(`[nio-abi] while encoding array, size mismatch array length ${length} does not equal ${value.length}`); }
...
function getParamCoder(typeInput) {
    ...
    const invalidTypeErrorMessage = `[nio-abi] while getting param coder (getParamCoder) type value ${JSON.stringify(type)} is either invalid or unsupported by nio-abi.`;
        ...
        switch (prefix) {
            case 'int': case 'uint':
                ...
                if (intSize === 0 || intSize > 256 || (intSize % 8) !== 0) {
                    throw new Error(`[nio-abi] while getting param coder for type ${type}, invalid ${prefix}<N> width: ${type}`);
                }
            ...
            case 'bytes':
                ...
                if (part[3]) {
                    ...
                    if (size === 0 || size > 32) {
                        throw new Error(`[nio-abi] while getting param coder for prefix bytes, invalid type ${type}, size ${size} should be 0 or greater than 32`);
                    }
...
const unitMap = {
    ...
    'nio':        '1000000000000000000', // eslint-disable-line
};
 ...
 * @param {String} unit the unit to convert to, default NIO
 * @returns {BigNumber} value of the unit (in Wei)
 * @throws error if the unit is not correct:w
 */
function getValueOfUnit(unitInput) {
    const unit = unitInput ? unitInput.toLowerCase() : 'nio';
    var unitValue = unitMap[unit]; // eslint-disable-line

    if (typeof unitValue !== 'string') {
        throw new Error(`[nio-unit] the unit provided ${unitInput} doesn't exists, please use the one of the following units ${JSON.stringify(unitMap, null, 2)}`);
    }
...
function toWei(nioInput, unit) {
    var nio = numberToString(nioInput); // eslint-disable-line
    const base = getValueOfUnit(unit);
    const baseLength = unitMap[unit].length - 1 || 1;

    // Is it negative?
    var negative = (nio.substring(0, 1) === '-'); // eslint-disable-line
    if (negative) {
        nio = nio.substring(1);
    }

    if (nio === '.') { throw new Error(`[nio-unit] while converting number ${nioInput} to wei, invalid value`); }

    // Split it into a whole and fractional part
    var comps = nio.split('.'); // eslint-disable-line
    if (comps.length > 2) { throw new Error(`[nio-unit] while converting number ${nioInput} to wei,  too many decimal points`); }

    var whole = comps[0], fraction = comps[1]; // eslint-disable-line

    if (!whole) { whole = '0'; }
    if (!fraction) { fraction = '0'; }
    if (fraction.length > baseLength) { throw new Error(`[nio-unit] while converting number ${nioInput} to wei, too many decimal places`); }
...
var fromNio = function(number) {
    ...
    return isBN(number) ? toWei(number, "nio") : toWei(number, "nio").toString(10);
};
...
var toNio = function(number) {
    ...
    return isBN(number) ? fromWei(number, 'nio') : fromWei(number, 'nio').toString(10);
};
